...
const Contact = () => {
  ...
  const handleSend = (e) => {
    ...
    if (clientName && email && EmailValidation(email) && messages) {
      ...
      setSuccessMsg(
        `مرحبا عزيزي ${clientName}, لقد تم إرسال رسائلك بنجاح. شكرا لك على وقتك!`
      );
      setClientName("");
      setEmail("");
      setMessages("");
  ...
  return (
    <div className="w-full">
      <Title title="ابقى" subTitle="على تواصل" />
      <div className="p-6 w-full flex flex-col md:flex-row justify-between gap-4 md:gap-10 lgl:gap-20">
        ...
        <div className="w-full lgl:w-1/2">
          <p className="flex justify-between lgl:gap-6 w-full text-md text-[#ccc] py-4 border-b-[1px] border-b-zinc-800">
            <span className="bg-designColor text-gray-700 text-sm font-titleFont font-medium px--1 rounded-md flex items-center justify-center">
              الايميل:
            </span>
            mer.maher.ramadan@gmail.com
          </p>
          <p className="flex justify-between w-full text-lg text-[#ccc] py-4 border-b-[1px] border-b-zinc-800">
            <span className="bg-designColor text-gray-700 text-sm font-titleFont font-medium px-2 rounded-md flex items-center justify-center">
              العمل الحر:
      ...
      <div className="w-full mt-10">
        <Title title="ارسال" subTitle="رسالة" />
        {seuccessMsg ? (
